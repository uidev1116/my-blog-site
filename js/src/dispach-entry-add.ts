...
export default ($unit: Unit) => {
  // targetがなければ実行しない
  if (0 === $unit.$target.size()) {
    return false;
  }

  const { Edit } = ACMS.Dispatch;
  let url = '';
  const focusableElements = 'a[href], area[href], input:not([disabled]), select:not([disabled]), textarea:not([disabled]), button:not([disabled]), object, embed, *[tabindex], *[contenteditable]';
  ...
  $.getJSON(url, (data) => {
    $.each(data.type, (i, type) => {
      ...
      const $input = $($.parseHTML(`<div class="acms-admin-inline-btn">
        <button type="button" aria-label="${data.label[i]}${ACMS.i18n("entry_editor.add_unit")}" class="${className ? className : 'acms-admin-btn-admin'}">
      ...
      $input.click(function () {
        ...
        $.get(url, function (html) {
          ...
          $unit.find(ACMS.Config.Edit.itemMark).each(function () {
            ...
            let max = parseInt($option.last().attr('value'), 10);
            let limit = size + $item.size() - $option.size();
