...
export default class MediaList extends React.Component<MediaListProp, MediaListState> {
  ...
  getOrderSelectBtnLabel() {
    ...
    } else if (orderActive === 'media_last_modified') {
      return `${ACMS.i18n("media.update_date")}（${active}）`;
    } else if (orderActive === 'media_datetime') {
      return `${ACMS.i18n("media.created_date")}（${active}）`;
    } else if (orderActive === 'media_path') {
      return `${ACMS.i18n("media.file_name")}（${active}）`
    } else if (orderActive === 'media_filesize') {
  ...
  toggleCheck(item) {
    ...
    if (radioMode) {
      ...
      if (findItem) {
        this.props.actions.updateMediaList(Object.assign({}, findItem, { checked: !findItem.checked }));
      }
    }
    this.props.actions.updateMediaList(Object.assign({}, item, { checked: !item.checked }));
  }
  ...
  toggleCheckFrom(fromItemId: string, toItem: MediaItem) {
    ...
    items.forEach((item, index) => {
      if (index >= minIndex && index <= maxIndex) {
        updateMediaList(Object.assign({}, item, { checked: nextChecked }));
      }
    });
  }
  ...
  toggleAllCheck() {
    ...
    if (all) {
      actions.setMediaList(items.map(item => Object.assign({}, item, {
        checked: true
      })));
    } else {
      actions.setMediaList(items.map(item => Object.assign({}, item, {
  ...
  fetchMediaList(config: FetchConfig = {}) {
    ...
    const override: FetchConfig = { order, tag, limit, filetype, fileext, owner };
    if (year && month) {
    ...
    const settings = Object.assign({}, config, override);
  ...
  componentDidUpdate() {
    const { state } = this;
    const { style, scale, order, orderActive, limit, filterMenuOpened, owner } = state;
    localStorage.setItem('acms-media-list', JSON.stringify({
  ...
  componentDidMount() {
    ...
    this.setState({ style, scale, order, orderActive, limit, filterMenuOpened, owner }, () => {
  ...
  render() {
    const {
      mode, lastPage, config, archives, tags, radioMode,
      total, items, loading: propLoading, selectedTags,
      extensions
    } = this.props;
    ...
    const {
      style, loading, deleting, editMode, scale, dropdown, orderDropdown,
      selectedLabels, tagAdded, items: stateItems, limit, menu, owner,
      order, orderActive, oldItemId, toggleAll, filterMenuOpened, filetype
    } = this.state;

    const checkedLength = this.getCheckedItemsLength();
    const canDelete = () => {
      if (mode === 'edit' && checkedLength) {
        return true;
      }
      return false;
    };
    ...
    return (
      <div className={mediaListStyle}>
        ...
        <div className="acms-admin-filter">
          <div className="acms-admin-form">
            <div className="acms-admin-filter-body">
              <div className="acms-admin-filter-group">
                ...
                <div className="acms-admin-filter-content acms-admin-filter-content-fit">
                  <div className="acms-admin-filter-item">
                    <label className="acms-admin-filter-item-name" htmlFor="filter-date">{ACMS.i18n("media.created_date")}：</label>
                  ...
                  <div className="acms-admin-filter-item acms-admin-filter-item-full">
                    <label htmlFor="filter-tag" className="acms-admin-filter-item-name">{ACMS.i18n("media.tag")}</label>
                    <div className="acms-admin-form-width-medium">
                      <Select multi value={selectedTags.map((selectedTag) => ({
                        label: selectedTag,
                        value: selectedTag
                      }))}
              ...
              {filterMenuOpened && <div id="search-detail">
                <div className="acms-admin-filter-inner">
                  <div className="acms-admin-filter-group">
                    ...
                    <div className="acms-admin-filter-content">
                      <div className="acms-admin-filter-item acms-admin-filter-item-full">
                        <label className="acms-admin-filter-item-name" htmlFor="filter-file-name">{ACMS.i18n("media.file_name")}</label>
                        <input type="text" placeholder={ACMS.i18n("media.search_by_filename")} id="filter-file-name" className="acms-admin-form-width-full" value={keyword} onInput={this.onKeywordInput.bind(this)} />
                      </div>
                    </div>
                  </div>
                  <div className="acms-admin-filter-group">
                    ...
                    <div className="acms-admin-filter-content">
                      <div className="acms-admin-filter-item">
                        <label className="acms-admin-filter-item-name" htmlFor="filter-sort">{ACMS.i18n("media.number_of_items")}</label>
                        <select
                          id="filter-sort"
                          value={limit}
                      ...
                      <div className="acms-admin-filter-item">
                        <label className="acms-admin-filter-item-name" htmlFor="filter-type">{ACMS.i18n("media.type")}</label>
                        <select
                          id="filter-type"
                      ...
                      <div className="acms-admin-filter-item">
                        <label htmlFor="filter-ext" className="acms-admin-filter-item-name">{ACMS.i18n("media.extension")}</label>
                        ...
                        >
                          ...
                          {extensions.map((extension) => {
                            return <option value={extension}>{extension.toLowerCase()}</option>
                          })}
                        </select>
                      </div>
                      <div className="acms-admin-filter-item">
                        <label htmlFor="filter-owner" className="acms-admin-form-checkbox acms-admin-margin-none">
                          <input id="filter-owner" type="checkbox" checked={owner}
                                 onChange={(e) => {
                                   this.setState({
          ...
          <div className="acms-admin-filter-footer">
            ...
            >{ACMS.i18n("media.search")}</button>

            <p className="acms-admin-filter-detail-btn"><a href="#search-detail" onClick={(e) => {
              e.preventDefault();
              this.setState({
                filterMenuOpened: !filterMenuOpened
              })
            }}><span className="acms-admin-icon-arrow-right"></span>{ACMS.i18n("media.detailed_search")}</a></p>
          </div>
        </div>
        <div className="acms-admin-media-action-group">
          <div className="acms-admin-media-action-item acms-admin-media-action-item-fix">
            ...
            <div className="acms-admin-btn-group acms-admin-media-action-display-switch" style={{ padding: '0px' }}>
              <button type="button" className={classnames("acms-admin-btn", {
                "acms-admin-btn-active": style === "table"
              })} onClick={this.changeStyle.bind(this, 'table')} disabled={style === 'table'}><i className="acms-admin-icon-list" /></button>
              <button type="button" className={classnames("acms-admin-btn", {
                "acms-admin-btn-active": style === "list"
              })} onClick={this.changeStyle.bind(this, 'list')} disabled={style === 'list'}><i className="acms-admin-icon-grid" /></button>
            </div>

            <div className="acms-admin-media-action-sort-select-wrap">
              <div className="acms-admin-media-action-sort-select">
                {style === 'table' && <button type="button" className="acms-admin-btn" onClick={this.showDropdownMenu}><i className="acms-admin-icon-config"></i></button>}
                {dropdown && <ul className="acms-admin-dropdown-menu" style={{ display: 'block', minWidth: '160px' }}>
                  ...
                  <li style={{ padding: '5px' }}>
                    <label className="acms-admin-form-checkbox">
                      ...
                      />
                      <i className="acms-admin-ico-checkbox" />
                      {ACMS.i18n("media.registered_date")}
                    </label>
                  </li>
                  <li style={{ padding: '5px' }}>
                    <label className="acms-admin-form-checkbox">
              ...
              <div className="acms-admin-media-action-sort-select" style={{position: 'relative'}}>
                <button type="button" className="acms-admin-select-btn" onClick={this.showOrderDropdown.bind(this)}>{this.getOrderSelectBtnLabel()}</button>
                {orderDropdown && <ul className="acms-admin-dropdown-menu" style={{ display: 'block', minWidth: '160px' }}>
                  <li style={{ padding: '5px' }}>
                    <label className="acms-admin-form-radio">
                      <input type="radio" checked={orderActive === 'media_title'} onChange={() => {
                  ...
                  <li style={{ padding: '5px' }}>
                    <label className="acms-admin-form-radio">
                      <input type="radio" checked={orderActive === 'media_last_modified'} onChange={() => {
                  ...
                  <li style={{ padding: '5px' }}>
                    <label className="acms-admin-form-radio">
                      <input type="radio" checked={orderActive === 'media_datetime'} onChange={() => {
                  ...
                  <li style={{ padding: '5px' }}>
                    <label className="acms-admin-form-radio">
                      <input type="radio" checked={orderActive === 'media_filesize'} onChange={() => {
                  ...
                  <li style={{ padding: '5px', borderTop: '1px solid #EEE' }}>
                    <label className="acms-admin-form-radio">
                      <input type="radio" checked={order[orderActive] === 'asc'} onChange={() => {
                  ...
                  <li style={{ padding: '5px' }}>
                    <label className="acms-admin-form-radio">
                      <input type="radio" checked={order[orderActive] === 'desc'} onChange={() => {
          ...
          <div className="acms-admin-media-action-item acms-admin-media-action-item-middle">
            ...
            <div className="acms-admin-media-action-range" >
              <span style={{display: 'inline-flex', width: '100%', alignItems: 'center'}}>
                <span style={{display: 'block', whiteSpace: 'nowrap', marginRight: '10px', fontSize: '13px'}} className="acms-admin-hide-visually">{ACMS.i18n("media.display_size")}</span>
                <i className="acms-admin-icon-unit-image" style={{fontSize: '12px', color: '#777777', marginRight: '10px'}}></i>
                <RangeSlider className="acms-admin-range-slider" min={1} max={2} step={0.1} value={scale} onChange={this.scaleChanged} />
                <i className="acms-admin-icon-unit-image" style={{fontSize: '18px', color: '#777777', marginLeft: '10px'}}></i>
              </span>
            </div>
            </>
            }
            ...
            <div className="acms-admin-media-action-select-all">
              <label onChange={this.toggleAllCheck.bind(this)}
                className="acms-admin-media-grid-checkbox-wrap">
                <div className={classnames("acms-admin-media-grid-checkbox", {
                  "selected": toggleAll
                })}></div>
                <input type="checkbox" />
        ...
        <div className="acms-admin-form-group acms-admin-media-action-item acms-admin-media-action-item-left">
          <div className="acms-admin-form">
            <select className="acms-admin-margin-right-mini" value={editMode} onChange={(e) => {
            ...
            {editMode === 'remove' && <Fragment>
              {ACMS.i18n("media.checked_items").replace("$1", checkedLength)}&nbsp;
              <button type="button" className="acms-admin-btn acms-admin-btn-danger" onClick={this.removeImgs.bind(this)}>{ACMS.i18n("media.remove")}</button>
            </Fragment>}
            {editMode === 'tag' && <Fragment>
              <div style={{ display: 'inline-block', verticalAlign: 'middle' }} className="acms-admin-margin-right-mini">
                {ACMS.i18n("media.add_tags_to_checked_items").replace("$1", checkedLength)}
              </div>
              <div className="acms-admin-select2" style={{ display: 'inline-block', width: '200px', verticalAlign: 'middle', marginRight: '5px' }}>
                <Creatable multi
              ...
              <button type="button" className="acms-admin-btn" onClick={this.addLabelToItems.bind(this)}>追加</button>
            </Fragment>}
          </div>
        ...
        <form ref={(list: HTMLFormElement) => { this.list = list; }} className="acms-admin-media-list" style={{ display: style === 'table' ? 'block' : 'none' }}>
              ...
              <thead className="acms-admin-table-heading acms-admin-media-heading">
                <tr>
                  <th className="acms-admin-media-row-check acms-admin-table-nowrap">
                    {mode === 'edit' &&
                      <label className="acms-admin-form-checkbox acms-admin-margin-none" onChange={this.toggleAllCheck.bind(this)}>
                        <input type="checkbox" />
                        <i className="acms-admin-ico-checkbox" />
                      </label>
                    }
                  </th>
                  {menu.image && <th className="acms-admin-media-row-image acms-admin-table-nowrap">{ACMS.i18n("media.image2")}</th>}
                  {menu.id && <th className="acms-admin-media-row-id acms-admin-table-nowrap">ID</th>}
                  {menu.filename && <th onClick={this.setOrder.bind(this, 'media_title')} className={`acms-admin-media-row-title ${this.getThClassName('media_title')}`}>{ACMS.i18n("media.file_name")}</th>}
                  {menu.tag && <th className="acms-admin-media-row-tag">{ACMS.i18n("media.tag")}</th>}
                  {menu.last_modified && <th onClick={this.setOrder.bind(this, 'media_last_modified')} className={`acms-admin-media-row-date acms-admin-table-nowrap ${this.getThClassName('media_last_modified')}`}>{ACMS.i18n("media.updated_date")}</th>}
                  {menu.upload_datetime && <th onClick={this.setOrder.bind(this, 'media_datetime')} className={`acms-admin-media-row-date acms-admin-table-nowrap ${this.getThClassName('media_datetime')}`}>{ACMS.i18n("media.created_date")}</th>}
                  {menu.filesize && <th onClick={this.setOrder.bind(this, 'media_filesize')} className={`acms-admin-media-row-size acms-admin-table-nowrap ${this.getThClassName('media_filesize')}`}>{ACMS.i18n("media.file_size")}</th>}
                  {menu.blogname && <th className="acms-admin-media-row-blogname">{ACMS.i18n("media.blog_name")}</th>}
                  {mode === 'edit' &&
                    <th className="acms-admin-media-row-action acms-admin-table-nowrap" style={{ whiteSpace: 'nowrap' }}>{ACMS.i18n("media.action")}</th>}
                </tr>
              </thead>
              <tbody>
              {items.map(item => (
                ...
                >
                  <td className="acms-admin-media-row-check acms-admin-table-nowrap">
                    {item.media_editable}
                    {(mode !== 'edit' || item.media_editable) && <label className="acms-admin-form-checkbox acms-admin-margin-none">
                      <input type="checkbox" name="checks[]" value={`${item.media_bid}:${item.media_id}`} checked={item.checked} onChange={this.toggleCheck.bind(this, item)} />
                      <i className="acms-admin-ico-checkbox" />
                    </label>}
                  </td>
                  {menu.image && <td className="acms-admin-media-row-image acms-admin-table-nowrap">
                    <ConditionalWrap
                      wrap={(chidren) => {
                        return (<a href={item.media_permalink} target="_blank" rel="noopener noreferrer">{chidren}</a>)
                      }}
                      condition={item.media_type === 'file'}
                    >
                      <div className="acms-admin-media-list-thumbnail-wrap">
                        <img src={`${item.media_thumbnail}?date=${item.media_datetime}`} className="acms-admin-media-list-thumbnail" />
                      </div>
                    </ConditionalWrap>
                  </td>}
                  {menu.id && <td className="acms-admin-media-row-id acms-admin-table-nowrap"><span className="acms-admin-label">{item.media_id}</span></td>}
                  {menu.filename && <td className="acms-admin-media-row-title">{item.media_title}</td>}
                  {menu.tag && <td className="acms-admin-media-row-tag">{item.media_label && item.media_label.split(delimiter).map(label => <span className="acms-admin-label acms-admin-label-info"
                    style={{
                      display: 'inline-block',
                      marginRight: '5px',
                      marginTop: '5px'
                  }}>{label}</span>)}</td>}
                  {menu.last_modified && <td className="acms-admin-media-row-date acms-admin-table-nowrap">{dayjs(item.media_last_modified).format('YYYY/MM/DD')}<br/>{dayjs(item.media_last_modified).format('HH:mm')}</td>}
                  {menu.upload_datetime && <td className="acms-admin-media-row-date acms-admin-table-nowrap">{dayjs(item.media_datetime).format('YYYY/MM/DD')}<br/>{dayjs(item.media_datetime).format('HH:mm')}</td>}
                  {menu.filesize && <td className="acms-admin-media-row-action acms-admin-table-nowrap">
                    {item.media_filesize && <>{formatBytes(item.media_filesize)}</>}
                    {(item.media_size && item.media_type === 'image') && <span style={{ display: 'block', marginTop: '5px' }}>{item.media_size}px</span>}
                  </td>}
                  {menu.blogname && <td className="acms-admin-media-row-blogname">{item.media_blog_name}</td>}
                  {mode === 'edit' &&
                    <td className="acms-admin-table-nowrap">
                      {item.media_editable &&
                        <button type="button" onClick={this.setItem.bind(this, item)} className="acms-admin-btn">{ACMS.i18n("media.edit")}</button>}
                    </td>}
                </tr>
              ))}
              </tbody>
        ...
        <div className="acms-admin-media-grid" style={{ display: style === 'table' ? 'none' : 'flex' }}>
          {items.map((item, index) => (
            <div className="acms-admin-media-grid-item" key={item.media_id} style={{ width: `${150 * scale}px`, height: `${150 * scale}px` }}>
              <div className={classnames('acms-admin-media-grid-figure', { selected: item.checked })} title={item.media_title} onClick={(e) => {
                if (e.shiftKey) {
                  this.toggleCheckFrom(oldItemId, item);
                } else {
                  this.toggleCheck(item);
                ...
                <div className="acms-admin-media-grid-shadow"></div>
                {item.media_type === 'file' && <div className="acms-admin-media-grid-img-wrap">
                  <img className="acms-admin-media-grid-file-img"
                ...
                {(item.media_type === 'image' || item.media_type === 'svg') &&
                  <img src={`${item.media_thumbnail}?date=${item.media_last_modified}`}
                ...
                <div className={classnames('acms-admin-media-grid-flame', { selected: item.checked })}>
                  <div className="acms-admin-media-grid-checkbox"></div>
        ...
        <div className="acms-admin-itemsAmount-container">
          <p>{((page - 1) * parseInt(limit, 10)) + 1}{ACMS.i18n("media.items")} - {page * parseInt(limit, 10) > total ? total : page * parseInt(limit, 10)}{ACMS.i18n("media.items")} / {ACMS.i18n("media.all")}{total}{ACMS.i18n("media.items")}</p>
        </div>
          ...
          {lastPage > 1 &&
            <ul className="acms-admin-pager">
              {(page > 1) && <li>
                ...
                }}>«&nbsp;{ACMS.i18n("media.prev")}</button>
              </li>}
              {(() => {
                const pages = [];
                for (let i = 0; i < lastPage; i++) {
                  pages.push(<li className={classnames({
                    cur: i + 1 === page
                  })}
                  >
                    <button
              ...
              {(page + 1 <= lastPage) && <li>
                <button
                  ...
                  }}>{ACMS.i18n("media.next")}&nbsp;»</button>
              </li>}
            </ul>
          }
        </div>
        {deleting && <Splash message={ACMS.i18n("media.deleting_media")} />}
        <Notify message={ACMS.i18n("media.add_tags_to_media_confirm")} show={tagAdded} onFinish={() => {
          this.setState({
            tagAdded: false
          });
        }} />
