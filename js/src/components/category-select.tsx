...
export default class CategorySelect extends Component<CategorySelectProp, CategorySelectState> {
  ...
  handleAddCategory(e) {
    ...
    var data = ACMS.Library.getPostData($form);
    $.ajax({
      'url': url,
      'type': 'post',
      'dataType': 'json',
  ...
  render() {
    const { creation, noneOption, targetDom, etcTargetDoms } = this.props;
    const { showModal, categoryAddParent, currentValue, addCategoryError } = this.state;
    ...
    return (
      <div>
        ...
        {creation && <div style={{ display: 'inline-block', verticalAlign: 'top', marginLeft: '3px'}} id="entry-create-category-display">
          <button type="button" className="acms-admin-btn-admin" style={{ padding: '8px 8px'}} onClick={this.handleShowModal}>追加</button>
        </div>}
        ...
        <ACMSModal
          ...
          footer={
            <div>
              <button className="acms-admin-btn" onClick={this.handleAddCategory}>追加</button>
            </div>
          }
        >
          <form className="acms-admin-form" style={{ marginTop: '20px' }} ref={this.addCategoryFormRef}>
            ...
            <div className="acms-admin-form-group">
              ...
              {!addCategoryError.nameRequired && <div role="alert" aria-live="assertive">
            ...
            <div className="acms-admin-form-group">
              ...
              <div role="alert" aria-live="assertive">
                {!addCategoryError.codeRequired && <div data-validator-label="validator-code-required" className="validator-result-0">
                  <p className="error-text">コードネームが入力されていません。</p>
                </div>}
                {!addCategoryError.codeDouble && <div data-validator-label="validator-code-double" className="validator-result-0">
                  <p className="error-text">既に使用されているコードネームです。</p>
                </div>}
                {!addCategoryError.codeReserved && <div data-validator-label="validator-code-reserved" className="validator-result-0">
            ...
            <div className="acms-admin-form-group">
              <label style={{ marginBottom: '3px', fontSize: '11px', color: '#666' }}>スコープ</label><br></br>
              ...
              <div className="acms-admin-form-checkbox">
                ...
                <label htmlFor="input-checkbox-global">
                  <i className="acms-admin-ico-checkbox"></i>下の階層のブログと共有する
                </label>
              </div>
              {!addCategoryError.scopeRequired && <div role="alert" aria-live="assertive">
