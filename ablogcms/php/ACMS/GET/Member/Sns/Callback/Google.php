...
class ACMS_GET_Member_Sns_Callback_Google extends ACMS_GET
{
    ...
    protected function getSubId(array $data): string
    {
        if (!isset($data['sub']) || empty($data['sub'])) {
            throw new \RuntimeException('GoogleアカウントのID取得に失敗しました');
        }
        return $data['sub'];
    }
    ...
    protected function getUserName(array $data): string
    {
        if (!isset($data['name']) || empty($data['name'])) {
            throw new \RuntimeException('Googleアカウント名の取得に失敗しました');
        }
        return $data['name'];
    }
    ...
    protected function oauth(): array
    {
        ...
        $accessToken = $googleApi->getAccessToken($code);
        if ($accessToken) {
            return $googleApi->verifyIdToken($accessToken);
        }
