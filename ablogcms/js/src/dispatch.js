...
export default (context) => {
  ...
  // 会員限定記事の表示・非表示
  ;(async () => {
    const membersOnlyEntryDom = document.querySelector(
      ACMS.Config.membersOnlyEntryMark,
    )
  ...
  // ログイン状態による表示・非表示
  ;(async () => {
    const loginHiddenDoms = document.querySelectorAll(
      ACMS.Config.loginHiddenMark,
    ) // ログイン状態の時、非表示にする
    const loginShowDoms = document.querySelectorAll(ACMS.Config.loginShowMark) // ログイン状態の時、表示する
    const logoutHiddenDoms = document.querySelectorAll(
      ACMS.Config.logoutHiddenMark,
    ) // ログアウト状態の時、非表示にする
  ...
  if (
    document.querySelector(ACMS.Config.scrollHintMark) ||
    document.querySelector(TableUnitScrollHint)
  ) {
    import(
      /* webpackChunkName: "scroll-hint-css" */ 'scroll-hint/css/scroll-hint.css'
    ).then(() => {
      // build in js
      new ScrollHint(ACMS.Config.scrollHintMark, ACMS.Config.scrollHintConfig) // eslint-disable-line no-new

      // table unit
      new ScrollHint(TableUnitScrollHint, {
  ...
  lazyLoad(
    ...
    (wrapper) => {
      ...
      const page =
        parseInt(elm.getAttribute(pdfPreviewConfig.pageAttr), 10) || 1
      const imageWidth =
        parseInt(elm.getAttribute(pdfPreviewConfig.widthAttr), 10) ||
        elm.clientWidth
  ...
  lazyLoad(
    ...
    (item) => {
      import(
        /* webpackChunkName: "open-street-map" */ './lib/open-street-map'
      ).then(({ default: openStreetMap }) => {
        openStreetMap(item)
      })
    },
  )
  ...
  lazyLoad(
    ...
    (item) => {
      import(
        /* webpackChunkName: "open-street-map" */ './lib/street-view'
      ).then(({ default: streetView }) => {
  ...
  ACMS.Dispatch.wysiwyg = {
    async init(elm) {
      const { default: wysiwyg } = await import(
        /* webpackChunkName: "wysiwyg" */ './lib/wysiwyg'
      )
  ...
  const dispatchFlatPicker = async (ctx) => {
    ...
    if (flatPickerTarget && flatPickerTarget.length) {
      const { default: flatPicker } = await import(
        /* webpackChunkName: "flatpickr" */ 'flatpickr'
      )
      import(
        /* webpackChunkName: "flatpickr-css" */ 'flatpickr/dist/flatpickr.min.css'
      )
      const options = {
      ...
      if (/^ja/.test(ACMS.i18n.lng)) {
        const lang = await import(
          /* webpackChunkName: "flatpickr-ja" */ 'flatpickr/dist/l10n/ja'
        )
    ...
    //-------------------
    // timepicker
    const flatTimePickerTarget = document.querySelectorAll(
      ACMS.Config.flatTimePicker,
    )
  ...
  //-------------------
  // contrast color
  const contrastColorTarget = document.querySelectorAll(
    ACMS.Config.contrastColorTarget,
  )
   ...
   */
  const passwordStrength = document.querySelectorAll(
    ACMS.Config.passwordStrengthMark,
  )
  if (passwordStrength.length > 0) {
    import(/* webpackChunkName: "zxcvbn" */ './lib/zxcvbn').then(
      ({ default: zxcvbn }) => {
  ...
  //-------------------
  // document-outliner
  const outlineTarget = document.querySelectorAll(
    ACMS.Config.documentOutlinerMark,
  )
  if (outlineTarget && outlineTarget.length) {
    ;[].forEach.call(outlineTarget, (item) => {
      requestAnimationFrame(() => {
        ...
        ;[].forEach.call(
          document.querySelectorAll(ACMS.Config.scrollToMark),
          (anchor) => {
