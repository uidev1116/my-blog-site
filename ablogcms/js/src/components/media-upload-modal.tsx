...
export default class MediaUploadModal extends Component<
  MediaUploadModalProp,
  MediaUploadModalState
> {
  static defaultProps = {
      ...
      ;[].forEach.call(items, (item: MediaUploadItem) => {
        ...
        const promise = new Promise((resolve) => {
          if (
            ...
            resizeImage
              .getBlobFromFile(file, resizeType, parseInt(largeSize, 10))
              .then(({ blob, resize }) => {
                const uploadData =
                  resize && ACMS.Config.mediaClientResize !== 'off'
                    ? blob
                    : file
                this.upload(uploadData, label, name, item).then(
                  (uploadedItem) => {
      ...
      <>
              ...
              <div className="acms-admin-media-upload-item">
                ...
                >
                  ...
                  <div className="acms-admin-media-upload-caption">
                    <p className="acms-admin-media-upload-caption-text">
                      {item.name}
                    </p>
                  </div>
                </div>
        ...
        {items.length === 0 && (
          <p style={{ textAlign: 'center' }}>
            {ACMS.i18n('media.no_staged_media')}
          </p>
        )}
