...
export default () => {
  ...
  const dispatchTimemachineMode = () => {
    ...
    if (!area) {
      document.body.insertAdjacentHTML(
        'beforeend',
        '<div id="acms-timemachine-area" style="position: fixed; top: 0; bottom: 0; left: 0; right: 0; z-index: 100017;"></div>',
      )
      area = document.querySelector('#acms-timemachine-area')
    }
    let hasHistoryDevice = false
    if (ACMS.Config.timemachinePreviewHasHistoryDevice === 'on') {
      hasHistoryDevice = true
    }
  ...
  ACMS.Ready(() => {
    ...
    if (previewTarget) {
      ...
      const defaultDevice =
        previewTarget.getAttribute('data-default-device') || ''
      let hasHistoryDevice = false
      if (previewTarget.getAttribute('data-has-history-device') === 'on') {
        hasHistoryDevice = true
      }
      /**
       * preview mode
      ...
      dispatchPreviewMode(
        previewTarget,
        previewTarget.getAttribute('data-url'),
        false,
        hasShareBtn,
        defaultDevice,
        hasHistoryDevice,
        ACMS.Config.previewDeviceHistoryKey.approval,
      )
    } else {
      if (previewButtons.length > 0) {
        ...
        ;[].forEach.call(previewButtons, (button) => {
          ...
          let hasHistoryDevice = false
          if (button.getAttribute('data-has-history-device') === 'on') {
            hasHistoryDevice = true
          }
          button.addEventListener('click', () => {
            dispatchPreviewMode(
              ...
              defaultDevice,
              hasHistoryDevice,
              ACMS.Config.previewDeviceHistoryKey.preview,
            )
          })
        })
      }
