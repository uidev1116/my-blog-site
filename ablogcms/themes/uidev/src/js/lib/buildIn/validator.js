...
class Validator {
      ...
      const event = () => {
        ...
        if (validator && validator.length > 0) {
          validator.forEach((obj) => {
            ...
            if (
              !operator.isFunction(obj.validator) ||
              obj.validator.substring(0, 4) === 'all_'
            ) {
              return
            }
            let valid = true
            obj.number = parseInt(number, 10)
  ...
  all(elm) {
    ...
    this.checked.forEach((obj) => {
      const input2 = Array.prototype.filter.call(
        inputs,
        (input) =>
          input.getAttribute('name') === obj.field ||
          input.getAttribute('name') === `${obj.field}[]`,
      )
    ...
    ;[].forEach.call(inputs, (val) => {
      ...
      if (validator && validator.length > 0) {
        validator.forEach((obj) => {
          ...
          if (matches) {
            obj.number = parseInt(matches[1], 10)
          }
          Validator.label(obj, valid)
          success = success && valid
        })
