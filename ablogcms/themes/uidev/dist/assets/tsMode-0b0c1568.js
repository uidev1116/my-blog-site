import { I, H as N } from './vendor-b4af7ed9.js'
/*!-----------------------------------------------------------------------------
    ...
    if ((t && typeof t == 'object') || typeof t == 'function')
      for (let n of K(t))
        !E.call(e, n) &&
          ...
          M(e, n, {
            get: () => t[n],
            enumerable: !(o = R(t, n)) || o.enumerable,
          })
    return e
  },
  H = (e, t, i) => (T(e, t, 'default'), i && T(i, t, 'default')),
  a = {}
H(a, N)
var V = class {
    constructor(e, t) {
    ...
    _getClient() {
      return (
        this._client ||
          (this._client = (async () => (
            (this._worker = a.editor.createWebWorker({
...
function D(e, t, i = 0) {
  ...
  if (((o += e.messageText), i++, e.next))
    for (const n of e.next) o += D(n, t, i)
  return o
}
function w(e) {
...
var _ = class {
    ...
    _textSpanToRange(e, t) {
      let i = e.getPositionAt(t.start),
        ...
        { lineNumber: n, column: c } = i,
        { lineNumber: u, column: s } = o
      return {
        startLineNumber: n,
        startColumn: c,
        endLineNumber: u,
  ...
  W = class {
    ...
    getOrCreateModel(e) {
      const t = a.Uri.parse(e),
        i = a.editor.getModel(t)
      if (i) return i
      if (this.isLibFile(t) && this._hasFetchedLibFiles)
        return a.editor.createModel(
          this._libFiles[t.path.slice(1)],
          'typescript',
          t,
        )
      const o = I.getExtraLibs()[e]
  ...
  j = class extends _ {
    constructor(e, t, i, o) {
      ...
      const n = (s) => {
          if (s.getLanguageId() !== i) return
          const l = () => {
            const { onlyVisible: h } = this._defaults.getDiagnosticsOptions()
            h
              ? s.isAttachedToEditor() && this._doValidate(s)
              : this._doValidate(s)
          }
          let g
        ...
        c = (s) => {
          ...
          this._listener[l] &&
            (this._listener[l].dispose(), delete this._listener[l])
        }
      this._disposables.push(a.editor.onDidCreateModel((s) => n(s))),
        this._disposables.push(a.editor.onWillDisposeModel(c)),
      ...
      const i = [],
        {
          ...
          noSuggestionDiagnostics: c,
        } = this._defaults.getDiagnosticsOptions()
      o || i.push(t.getSyntacticDiagnostics(e.uri.toString())),
        n || i.push(t.getSemanticDiagnostics(e.uri.toString())),
        c || i.push(t.getSuggestionDiagnostics(e.uri.toString()))
      const u = await Promise.all(i)
      if (!u || e.isDisposed()) return
      const s = u
          .reduce((g, d) => d.concat(g), [])
          .filter(
            (g) =>
              (
                this._defaults.getDiagnosticsOptions()
        ...
        l = s
          .map((g) => g.relatedInformation || [])
          .reduce((g, d) => d.concat(g), [])
          .map((g) => (g.file ? a.Uri.parse(g.file.fileName) : null))
      await this._libFiles.fetchLibFilesIfNecessary(l),
        !e.isDisposed() &&
          a.editor.setModelMarkers(
            e,
            this._selector,
    ...
    _convertDiagnostics(e, t) {
      const i = t.start || 0,
        o = t.length || 1,
        { lineNumber: n, column: c } = e.getPositionAt(i),
        { lineNumber: u, column: s } = e.getPositionAt(i + o),
        l = []
      return (
        t.reportsUnnecessary && l.push(a.MarkerTag.Unnecessary),
        t.reportsDeprecated && l.push(a.MarkerTag.Deprecated),
        {
    ...
    _convertRelatedInformation(e, t) {
      ...
      return (
        t.forEach((o) => {
          let n = e
          if (
            (o.file && (n = this._libFiles.getOrCreateModel(o.file.fileName)),
            !n)
          )
            return
          const c = o.start || 0,
            u = o.length || 1,
            { lineNumber: s, column: l } = n.getPositionAt(c),
            { lineNumber: g, column: d } = n.getPositionAt(c + u)
          i.push({
            resource: n.uri,
            startLineNumber: s,
            startColumn: l,
...
`,
  ...
  B = class C extends _ {
    ...
    async provideCompletionItems(t, i, o, n) {
      const c = t.getWordUntilPosition(i),
        u = new a.Range(i.lineNumber, c.startColumn, i.lineNumber, c.endColumn),
        s = t.uri,
        l = t.getOffsetAt(i),
        g = await this._worker(s)
      if (t.isDisposed()) return
      const d = await g.getCompletionsAtPosition(s.toString(), l)
      return !d || t.isDisposed()
        ? void 0
        : {
            suggestions: d.entries.map((h) => {
              let y = u
              if (h.replacementSpan) {
                const S = t.getPositionAt(h.replacementSpan.start),
                  x = t.getPositionAt(
                    h.replacementSpan.start + h.replacementSpan.length,
                  )
                y = new a.Range(S.lineNumber, S.column, x.lineNumber, x.column)
              }
              const v = []
              return (
                h.kindModifiers !== void 0 &&
                  h.kindModifiers.indexOf('deprecated') !== -1 &&
                  v.push(a.languages.CompletionItemTag.Deprecated),
                {
                  uri: s,
                  position: i,
                  offset: l,
    ...
    async resolveCompletionItem(t, i) {
      const o = t,
        ...
        u = o.offset,
        l = await (
          await this._worker(n)
        ).getCompletionEntryDetails(n.toString(), u, o.label)
      return l
        ? {
            uri: n,
            position: c,
            ...
            documentation: { value: C.createDocumentationString(l) },
          }
        : o
    }
...
function P(e) {
  ...
  if (e.name === 'param' && e.text) {
    const [i, ...o] = e.text
    ;(t += `\`${i.text}\``),
      o.length > 0 && (t += ` — ${o.map((n) => n.text).join(' ')}`)
  } else
    Array.isArray(e.text)
      ? (t += ` — ${e.text.map((i) => i.text).join(' ')}`)
      : e.text && (t += ` — ${e.text}`)
  return t
}
var U = class L extends _ {
    ...
    async provideSignatureHelp(t, i, o, n) {
      ...
      return (
        l.items.forEach((d) => {
          const b = { label: '', parameters: [] }
          ;(b.documentation = { value: w(d.documentation) }),
            (b.label += w(d.prefixDisplayParts)),
            d.parameters.forEach((h, y, v) => {
              const S = w(h.displayParts),
                x = { label: S, documentation: { value: w(h.documentation) } }
              ;(b.label += S),
                b.parameters.push(x),
                y < v.length - 1 && (b.label += w(d.separatorDisplayParts))
            }),
            (b.label += w(d.suffixDisplayParts)),
            g.signatures.push(b)
        }),
        { value: g, dispose() {} }
      )
  ...
  $ = class extends _ {
    async provideHover(e, t, i) {
      const o = e.uri,
        n = e.getOffsetAt(t),
        c = await this._worker(o)
      if (e.isDisposed()) return
      const u = await c.getQuickInfoAtPosition(o.toString(), n)
      if (!u || e.isDisposed()) return
      const s = w(u.documentation),
        l = u.tags
          ? u.tags.map((d) => P(d)).join(`  
...
` + l
  ...
  z = class extends _ {
    async provideDocumentHighlights(e, t, i) {
      const o = e.uri,
        ...
        c = await this._worker(o)
      if (e.isDisposed()) return
      const u = await c.getDocumentHighlights(o.toString(), n, [o.toString()])
      if (!(!u || e.isDisposed()))
        return u.flatMap((s) =>
          s.highlightSpans.map((l) => ({
            range: this._textSpanToRange(e, l.textSpan),
            kind:
              l.kind === 'writtenReference'
                ? a.languages.DocumentHighlightKind.Write
                : a.languages.DocumentHighlightKind.Text,
          })),
        )
    }
  },
  G = class extends _ {
    ...
    async provideDefinition(e, t, i) {
      ...
      for (let l of u) {
        ...
        g && s.push({ uri: g.uri, range: this._textSpanToRange(g, l.textSpan) })
      }
      return s
    }
  },
  J = class extends _ {
    ...
    async provideReferences(e, t, i, o) {
      ...
      for (let g of s) {
        ...
        d && l.push({ uri: d.uri, range: this._textSpanToRange(d, g.textSpan) })
      }
      return l
    }
  },
    ...
    async provideDocumentSymbols(e, t) {
      ...
      const c = (s, l) => {
        ...
        return {
          ...
          tags: [],
          children:
            (d = s.childItems) == null ? void 0 : d.map((b) => c(b, s.text)),
          containerName: l,
        }
      }
  ...
  f =
    ...
    (p.enum = 'enum'),
    (p.variable = 'var'),
    (p.localVariable = 'local var'),
    (p.function = 'function'),
    (p.localFunction = 'local function'),
    (p.memberFunction = 'method'),
    (p.memberGetAccessor = 'getter'),
    (p.memberSetAccessor = 'setter'),
    (p.memberVariable = 'property'),
    (p.constructorImplementation = 'constructor'),
    (p.callSignature = 'call'),
    (p.indexSignature = 'index'),
    (p.constructSignature = 'construct'),
    (p.parameter = 'parameter'),
    (p.typeParameter = 'type parameter'),
    (p.primitiveType = 'primitive type'),
    (p.label = 'label'),
    (p.alias = 'alias'),
    (p.const = 'const'),
    (p.let = 'let'),
  ...
  q = class extends k {
    ...
    async provideDocumentRangeFormattingEdits(e, t, i, o) {
      const n = e.uri,
        ...
        u = e.getOffsetAt({ lineNumber: t.endLineNumber, column: t.endColumn }),
  ...
  Y = class extends k {
    async provideCodeActions(e, t, i, o) {
      const n = e.uri,
        ...
        u = e.getOffsetAt({ lineNumber: t.endLineNumber, column: t.endColumn }),
        s = k._convertOptions(e.getOptions()),
        l = i.markers
          .filter((h) => h.code)
          .map((h) => h.code)
          .map(Number),
        g = await this._worker(n)
      if (e.isDisposed()) return
      const d = await g.getCodeFixesAtPosition(n.toString(), c, u, l, s)
      return !d || e.isDisposed()
        ? { actions: [], dispose: () => {} }
        : {
            actions: d
              .filter((h) => h.changes.filter((y) => y.isNewFile).length === 0)
  ...
  Z = class extends _ {
    ...
    async provideRenameEdits(e, t, i, o) {
      const n = e.uri,
        ...
        s = await this._worker(n)
      if (e.isDisposed()) return
      const l = await s.getRenameInfo(c, u, { allowRenameOfImportPath: !1 })
      if (l.canRename === !1)
        return { edits: [], rejectReason: l.localizedErrorMessage }
      if (l.fileToRename !== void 0)
        throw new Error('Renaming files is not supported.')
      const g = await s.findRenameLocations(c, u, !1, !1, !1)
      ...
      for (const b of g) {
        ...
        if (h)
          d.push({
            resource: h.uri,
            versionId: void 0,
            textEdit: { range: this._textSpanToRange(h, b.textSpan), text: i },
          })
  ...
  ee = class extends _ {
    async provideInlayHints(e, t, i) {
      ...
      return e.isDisposed()
        ? null
        : {
            hints: (await s.provideInlayHints(n, c, u)).map((d) => ({
              ...d,
              label: d.text,
...
function ne() {
  return new Promise((e, t) => {
    if (!A) return t('JavaScript not registered!')
    e(A)
  })
}
function ae() {
...
function O(e, t) {
  const i = [],
    o = new V(t, e),
    n = (...s) => o.getLanguageServiceWorker(...s),
    c = new W(n)
  function u() {
    const { modeConfiguration: s } = e
    te(i),
      s.completionItems &&
        i.push(a.languages.registerCompletionItemProvider(t, new B(n))),
      s.signatureHelp &&
        i.push(a.languages.registerSignatureHelpProvider(t, new U(n))),
      s.hovers && i.push(a.languages.registerHoverProvider(t, new $(n))),
      s.documentHighlights &&
        i.push(a.languages.registerDocumentHighlightProvider(t, new z(n))),
      s.definitions &&
        i.push(a.languages.registerDefinitionProvider(t, new G(c, n))),
      s.references &&
        i.push(a.languages.registerReferenceProvider(t, new J(c, n))),
      s.documentSymbols &&
        i.push(a.languages.registerDocumentSymbolProvider(t, new Q(n))),
      s.rename && i.push(a.languages.registerRenameProvider(t, new Z(c, n))),
      s.documentRangeFormattingEdits &&
        i.push(
          a.languages.registerDocumentRangeFormattingEditProvider(t, new q(n)),
        ),
      s.onTypeFormattingEdits &&
        i.push(a.languages.registerOnTypeFormattingEditProvider(t, new X(n))),
      s.codeActions &&
        i.push(a.languages.registerCodeActionProvider(t, new Y(n))),
      s.inlayHints &&
        i.push(a.languages.registerInlayHintsProvider(t, new ee(n))),
