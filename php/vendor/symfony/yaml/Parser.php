        ...
        do {
            ...
            if ('-' === $this->currentLine[0] && self::preg_match('#^\-((?P<leadspaces>\s+)(?P<value>.+))?$#u', rtrim($this->currentLine), $values)) {
                ...
                if (isset($values['value']) && 0 === strpos(ltrim($values['value'], ' '), '-')) {
                    ...
                    $sequenceYaml = substr($this->currentLine, $sequenceIndentation ?? 0);
        ...
        try {
            ...
            switch ($value[0] ?? '') {
                ...
                case "'":
                    ...
                    if (isset($this->currentLine[$cursor]) && preg_replace('/\s*(#.*)?$/A', '', substr($this->currentLine, $cursor ?? 0))) {
                        throw new ParseException(sprintf('Unexpected characters near "%s".', substr($this->currentLine, $cursor ?? 0)));
                    }
