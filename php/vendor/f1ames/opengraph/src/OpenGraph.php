...
class OpenGraph implements Iterator
{
    ...
    static private function _parse($HTML) {
        ...
        if (!preg_match('@<\s*meta\s*charset\s*=\s*[\'"]([^\'"]*)[\'"][^>]*>@i', $HTML)) {
            $HTML = preg_replace('@(?=<\s*/\s*head[^\w]*>)@i', "<meta charset=\"UTF-8\"><meta http-equiv=\"Content-Type\" content=\"text/html; charset=UTF-8\">", $HTML);
        }
        if ( $enc = mb_detect_encoding($HTML, 'UTF-8, EUC-JP, SJIS, EUCJP-win, SJIS-win') ) {
